...
def results(i):
    ...
    return r,total,c
PAGE='''<!doctype html><meta charset="utf-8"><style>body{font:16px system-ui;max-width:900px;margin:30px auto}.card{border:1px solid #ccc;padding:12px;margin:12px 0}input,select,button{padding:8px;font:inherit}input{width:90%}.import{background:#eef5ff;padding:14px;border:1px solid #9ab}.danger{background:#a22;color:white}.ok{background:#e6f6e9;padding:10px}.error{background:#fee;padding:10px}</style><h1>Encuestas Gambling</h1><p>Construye la encuesta, ejecútala sobre 10.000 perfiles Persona8B y consulta resultados agregados. Hermes produce una respuesta por perfil.</p>{message}<form method="post" action="/create"><p>Identificador <input name="id" value="mi-encuesta"></p><p>Título <input id="title" name="title" value="Encuesta de apuestas deportivas"></p><div class=import><strong>1. Cargar un cuestionario completo</strong><p>Selecciona un fichero <code>.json</code> o <code>.xml</code>. Sus preguntas y opciones aparecerán abajo para revisarlas o editarlas.</p><input id="file" type="file" accept=".json,.xml,application/json,text/xml,application/xml"><p><small>Ejemplos: <code>ejemplo_cuestionario.json</code> y <code>ejemplo_cuestionario.xml</code>, en la carpeta del proyecto.</small></p></div><input id="payload" name="payload" type="hidden"><div id="qs"></div><button type="button" onclick="add()">+ Nueva pregunta</button><button type="button" class="danger" onclick="if(confirm('¿Limpiar?')){q=[];draw()}">Limpiar encuesta</button><button>Preparar encuesta</button></form><script>let q=[];function esc(x){return String(x).replaceAll('&','&amp;').replaceAll('"','&quot;')}function add(x={prompt:'',type:'single_choice',options:['','']}){q.push(x);draw()}function draw(){qs.innerHTML=q.map((x,i)=>`<div class=card><button type=button class=danger onclick="q.splice(${i},1);draw()">Quitar pregunta</button><p><input placeholder="Pregunta" value="${esc(x.prompt)}" oninput="q[${i}].prompt=this.value"></p><select onchange="q[${i}].type=this.value"><option value=single_choice ${x.type==='single_choice'?'selected':''}>Opción única</option><option value=multi_choice ${x.type==='multi_choice'?'selected':''}>Múltiples respuestas</option></select>${x.options.map((o,j)=>`<p><input placeholder="Opción" value="${esc(o)}" oninput="q[${i}].options[${j}]=this.value"><button type=button onclick="q[${i}].options.splice(${j},1);draw()">Quitar</button></p>`).join('')}<button type=button onclick="q[${i}].options.push('');draw()">+ Opción</button></div>`).join('')}function txt(n,s){let x=n.querySelector(s);return x?x.textContent.trim():''}function xml(text){let d=new DOMParser().parseFromString(text,'application/xml');if(d.querySelector('parsererror'))throw Error('XML mal formado');let root=d.documentElement;if(root.tagName!=='survey')throw Error('La raíz XML debe ser <survey>');let questions=[...root.querySelectorAll(':scope > question')].map(x=>({prompt:txt(x,'prompt'),type:x.getAttribute('type')||'single_choice',options:[...x.querySelectorAll(':scope > options > option')].map(o=>o.textContent.trim())}));if(!questions.length)throw Error('Falta al menos un elemento <question>');return {title:root.getAttribute('title')||txt(root,'title'),questions}}function normalize(d){if(!Array.isArray(d.questions))throw Error('Falta questions');return {title:d.title||title.value,questions:d.questions.map(x=>({prompt:x.prompt||'',type:x.type||'single_choice',options:(x.options||[]).map(o=>typeof o==='string'?o:o.label||'')}))}}file.onchange=async e=>{try{let f=e.target.files[0],t=await f.text(),d=f.name.toLowerCase().endsWith('.xml')?xml(t):JSON.parse(t);d=normalize(d);title.value=d.title;q=d.questions;draw();alert('Cuestionario cargado: '+q.length+' preguntas. Revísalo y pulsa Preparar encuesta.')}catch(err){alert('No se pudo cargar el cuestionario: '+err.message)}};document.querySelector('form').onsubmit=e=>{if(!q.length){alert('Añade una pregunta');e.preventDefault()}payload.value=JSON.stringify({title:title.value,questions:q})};add({prompt:'¿Qué valor tendría para ti una herramienta de análisis de cuotas?',type:'single_choice',options:['Ninguno','Bajo','Moderado','Alto']})</script>'''
